import "server-only";

import { desc, eq } from "drizzle-orm";
import { drizzle } from "drizzle-orm/postgres-js";
import postgres from "postgres";
import { createClient } from "redis";
import {
  type CreditBalance,
  type CreditTransaction,
  creditBalance,
  creditTransaction,
  type GuestSession,
  guestSession,
} from "../db/schema";
import { ChatSDKError } from "../errors";

// biome-ignore lint: Forbidden non-null assertion.
const client = postgres(process.env.POSTGRES_URL!);
const db = drizzle(client);

// Retry configuration
const MAX_RETRIES = 3;
const INITIAL_RETRY_DELAY = 100; // milliseconds

/**
 * Sleep for a specified duration
 */
function sleep(ms: number): Promise<void> {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

/**
 * Retry a database operation with exponential backoff
 *
 * @param operation - The async operation to retry
 * @param context - Context information for logging
 * @returns The result of the operation
 * @throws ChatSDKError if all retries fail
 */
async function retryDatabaseOperation<T>(
  operation: () => Promise<T>,
  context: string
): Promise<T> {
  let lastError: Error | null = null;

  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      return await operation();
    } catch (error) {
      lastError = error as Error;

      // Don't retry on business logic errors (ChatSDKError)
      if (error instanceof ChatSDKError) {
        throw error;
      }

      // Log the retry attempt
      console.warn(
        `Database operation failed (attempt ${attempt}/${MAX_RETRIES}): ${context}`,
        {
          error: error instanceof Error ? error.message : String(error),
          attempt,
        }
      );

      // Don't sleep after the last attempt
      if (attempt < MAX_RETRIES) {
        // Exponential backoff: 100ms, 200ms, 400ms
        const delay = INITIAL_RETRY_DELAY * 2 ** (attempt - 1);
        await sleep(delay);
      }
    }
  }

  // All retries failed
  console.error(
    `Database operation failed after ${MAX_RETRIES} attempts: ${context}`,
    {
      error: lastError instanceof Error ? lastError.message : String(lastError),
    }
  );

  throw new ChatSDKError(
    "bad_request:database",
    `Database operation failed after ${MAX_RETRIES} attempts: ${context}`
  );
}

// Redis client for guest session management and caching
let redisClient: ReturnType<typeof createClient> | null = null;

async function getRedisClient() {
  if (!process.env.REDIS_URL) {
    return null;
  }

  if (!redisClient) {
    redisClient = createClient({
      url: process.env.REDIS_URL,
    });

    redisClient.on("error", (err) => {
      console.error("Redis Client Error:", err);
    });

    await redisClient.connect();
  }

  return redisClient;
}

// Cache configuration for credit balances
const BALANCE_CACHE_TTL = 60; // 60 seconds (short TTL for frequently changing data)

/**
 * Get cache key for user balance
 */
function getBalanceCacheKey(userId: string): string {
  return `credit:balance:${userId}`;
}

/**
 * Invalidate balance cache for a user
 */
async function invalidateBalanceCache(userId: string): Promise<void> {
  try {
    const redis = await getRedisClient();
    if (redis) {
      const key = getBalanceCacheKey(userId);
      await redis.del(key);
      console.log("Balance cache invalidated", { userId });
    }
  } catch (error) {
    // Log but don't throw - cache invalidation failure shouldn't break operations
    console.error("Failed to invalidate balance cache", {
      userId,
      error: error instanceof Error ? error.message : String(error),
    });
  }
}

/**
 * Calculate credits for a given text output.
 * Formula: credits = (character_count / 20) rounded to 2 decimal places
 *
 * @param text - The text to calculate credits for
 * @returns The calculated credits, always >= 0 and rounded to 2 decimal places
 */
export function calculateCredits(text: string): number {
  try {
    // Validate input
    if (typeof text !== "string") {
      console.error("Invalid input to calculateCredits: expected string", {
        type: typeof text,
        value: text,
      });
      // Fallback: treat as empty string
      text = "";
    }

    // Count all characters including whitespace, punctuation, and Unicode
    const characterCount = text.length;

    // Apply formula: credits = character_count / 20
    const rawCredits = characterCount / 20;

    // Check for invalid calculation results
    if (Number.isNaN(rawCredits) || !Number.isFinite(rawCredits)) {
      console.error("Invalid credit calculation result", {
        text: text.substring(0, 100), // Log first 100 chars
        characterCount,
        rawCredits,
      });
      // Fallback: return 0
      return 0;
    }

    // Round to 2 decimal places
    const roundedCredits = Math.round(rawCredits * 100) / 100;

    // Ensure non-negative
    const finalCredits = Math.max(0, roundedCredits);

    // Log calculation for audit purposes
    if (characterCount > 0) {
      console.log("Credit calculation", {
        characterCount,
        calculatedCredits: finalCredits,
      });
    }

    return finalCredits;
  } catch (error) {
    // Catch any unexpected errors in calculation
    console.error("Unexpected error in calculateCredits", {
      error: error instanceof Error ? error.message : String(error),
      text: text?.substring(0, 100),
    });
    // Fallback: return 0 credits
    return 0;
  }
}

/**
 * Get credit balance for an authenticated user.
 * Uses Redis cache with 60-second TTL for performance.
 *
 * @param userId - The user's ID
 * @returns The user's credit balance
 * @throws ChatSDKError if the user doesn't have a credit balance record
 */
export async function getBalance(userId: string): Promise<CreditBalance> {
  return retryDatabaseOperation(async () => {
    try {
      const redis = await getRedisClient();

      // Try to get from cache first
      if (redis) {
        const cacheKey = getBalanceCacheKey(userId);
        const cachedBalance = await redis.get(cacheKey);
        if (cachedBalance) {
          console.log("Credit balance retrieved from cache", { userId });
          return JSON.parse(cachedBalance) as CreditBalance;
        }
      }

      // If not in cache, fetch from database
      const [balance] = await db
        .select()
        .from(creditBalance)
        .where(eq(creditBalance.userId, userId));

      if (!balance) {
        console.warn("Credit balance not found for user", { userId });
        throw new ChatSDKError(
          "not_found:database",
          "Credit balance not found for user"
        );
      }

      // Cache the result if Redis is available
      if (redis) {
        const cacheKey = getBalanceCacheKey(userId);
        await redis.set(cacheKey, JSON.stringify(balance), {
          EX: BALANCE_CACHE_TTL,
        });
        console.log("Credit balance cached in Redis", { userId });
      }

      return balance;
    } catch (error) {
      if (error instanceof ChatSDKError) {
        throw error;
      }
      console.error("Failed to get credit balance", {
        userId,
        error: error instanceof Error ? error.message : String(error),
      });
      throw new ChatSDKError(
        "bad_request:database",
        "Failed to get credit balance"
      );
    }
  }, `getBalance for user ${userId}`);
}

/**
 * Get credit balance for a guest session.
 *
 * @param sessionId - The guest session ID
 * @returns The guest session's credit balance
 * @throws ChatSDKError if the session doesn't exist
 */
export async function getGuestBalance(sessionId: string): Promise<number> {
  return retryDatabaseOperation(async () => {
    try {
      const [session] = await db
        .select()
        .from(guestSession)
        .where(eq(guestSession.sessionId, sessionId));

      if (!session) {
        console.warn("Guest session not found", { sessionId });
        throw new ChatSDKError("not_found:database", "Guest session not found");
      }

      // Convert decimal string to number
      return Number.parseFloat(session.balance);
    } catch (error) {
      if (error instanceof ChatSDKError) {
        throw error;
      }
      console.error("Failed to get guest balance", {
        sessionId,
        error: error instanceof Error ? error.message : String(error),
      });
      throw new ChatSDKError(
        "bad_request:database",
        "Failed to get guest balance"
      );
    }
  }, `getGuestBalance for session ${sessionId}`);
}

/**
 * Deduct credits from an authenticated user's balance.
 * Uses database transaction with row-level locking to prevent race conditions.
 *
 * @param params - Deduction parameters
 * @param params.userId - The user's ID
 * @param params.amount - The amount of credits to deduct
 * @param params.description - Description of the transaction
 * @param params.metadata - Optional metadata for the transaction
 * @returns The created credit transaction record
 * @throws ChatSDKError if insufficient credits or database error
 */
export async function deductCredits(params: {
  userId: string;
  amount: number;
  description: string;
  metadata?: Record<string, unknown>;
}): Promise<CreditTransaction> {
  const { userId, amount, description, metadata } = params;

  // Validate amount is positive
  if (amount < 0) {
    console.warn("Attempted to deduct negative credits", { userId, amount });
    throw new ChatSDKError(
      "bad_request:database",
      "Credit deduction amount must be positive"
    );
  }

  // Round amount to 2 decimal places
  const roundedAmount = Math.round(amount * 100) / 100;

  return retryDatabaseOperation(async () => {
    try {
      // Use database transaction with row-level locking
      const result = await db.transaction(async (tx) => {
        // Lock the user's credit balance row for update
        const [balance] = await tx
          .select()
          .from(creditBalance)
          .where(eq(creditBalance.userId, userId))
          .for("update");

        if (!balance) {
          console.warn("Credit balance not found during deduction", { userId });
          throw new ChatSDKError(
            "not_found:database",
            "Credit balance not found for user"
          );
        }

        // Convert balance to number for comparison
        const currentBalance = Number.parseFloat(balance.balance);

        // Validate sufficient balance
        if (currentBalance < roundedAmount) {
          console.warn("Insufficient credits for deduction", {
            userId,
            currentBalance,
            requestedAmount: roundedAmount,
          });
          throw new ChatSDKError(
            "bad_request:database",
            `Insufficient credits. Current balance: ${currentBalance}, Required: ${roundedAmount}`
          );
        }

        // Calculate new balance
        const newBalance =
          Math.round((currentBalance - roundedAmount) * 100) / 100;

        // Update credit balance
        await tx
          .update(creditBalance)
          .set({
            balance: newBalance.toFixed(2),
            updatedAt: new Date(),
          })
          .where(eq(creditBalance.userId, userId));

        // Record transaction
        const [transaction] = await tx
          .insert(creditTransaction)
          .values({
            userId,
            type: "deduction",
            amount: roundedAmount.toFixed(2),
            balanceAfter: newBalance.toFixed(2),
            description,
            metadata: metadata ? metadata : null,
          })
          .returning();

        console.log("Credits deducted successfully", {
          userId,
          amount: roundedAmount,
          newBalance,
          transactionId: transaction.id,
        });

        return transaction;
      });

      // Invalidate cache after successful deduction
      await invalidateBalanceCache(userId);

      return result;
    } catch (error) {
      if (error instanceof ChatSDKError) {
        throw error;
      }
      console.error("Failed to deduct credits", {
        userId,
        amount: roundedAmount,
        error: error instanceof Error ? error.message : String(error),
      });
      throw new ChatSDKError(
        "bad_request:database",
        "Failed to deduct credits"
      );
    }
  }, `deductCredits for user ${userId}, amount ${roundedAmount}`);
}

/**
 * Deduct credits from a guest session.
 * Updates Redis session storage if available, otherwise falls back to database.
 *
 * @param sessionId - The guest session ID
 * @param amount - The amount of credits to deduct
 * @returns The new balance after deduction
 * @throws ChatSDKError if insufficient credits or session not found
 */
export async function deductGuestCredits(
  sessionId: string,
  amount: number
): Promise<number> {
  // Validate amount is positive
  if (amount < 0) {
    console.warn("Attempted to deduct negative guest credits", {
      sessionId,
      amount,
    });
    throw new ChatSDKError(
      "bad_request:database",
      "Credit deduction amount must be positive"
    );
  }

  // Round amount to 2 decimal places
  const roundedAmount = Math.round(amount * 100) / 100;

  return retryDatabaseOperation(async () => {
    try {
      const redis = await getRedisClient();

      if (redis) {
        // Use Redis for guest session storage
        const key = `guest:${sessionId}`;
        const sessionData = await redis.get(key);

        if (!sessionData) {
          console.warn("Guest session not found in Redis", { sessionId });
          throw new ChatSDKError(
            "not_found:database",
            "Guest session not found"
          );
        }

        const session = JSON.parse(sessionData) as GuestSession;
        const currentBalance = Number.parseFloat(session.balance);

        // Validate sufficient balance
        if (currentBalance < roundedAmount) {
          console.warn("Insufficient guest credits", {
            sessionId,
            currentBalance,
            requestedAmount: roundedAmount,
          });
          throw new ChatSDKError(
            "bad_request:database",
            `Insufficient credits. Current balance: ${currentBalance}, Required: ${roundedAmount}`
          );
        }

        // Calculate new balance
        const newBalance =
          Math.round((currentBalance - roundedAmount) * 100) / 100;

        // Update session in Redis
        session.balance = newBalance.toFixed(2);
        await redis.set(key, JSON.stringify(session), {
          EX: 24 * 60 * 60, // 24 hours TTL
        });

        console.log("Guest credits deducted successfully (Redis)", {
          sessionId,
          amount: roundedAmount,
          newBalance,
        });

        return newBalance;
      }

      // Fallback to database if Redis is not available
      console.warn(
        "Redis unavailable, using database fallback for guest session",
        {
          sessionId,
        }
      );

      const result = await db.transaction(async (tx) => {
        // Lock the guest session row for update
        const [session] = await tx
          .select()
          .from(guestSession)
          .where(eq(guestSession.sessionId, sessionId))
          .for("update");

        if (!session) {
          console.warn("Guest session not found in database", { sessionId });
          throw new ChatSDKError(
            "not_found:database",
            "Guest session not found"
          );
        }

        const currentBalance = Number.parseFloat(session.balance);

        // Validate sufficient balance
        if (currentBalance < roundedAmount) {
          console.warn("Insufficient guest credits (database)", {
            sessionId,
            currentBalance,
            requestedAmount: roundedAmount,
          });
          throw new ChatSDKError(
            "bad_request:database",
            `Insufficient credits. Current balance: ${currentBalance}, Required: ${roundedAmount}`
          );
        }

        // Calculate new balance
        const newBalance =
          Math.round((currentBalance - roundedAmount) * 100) / 100;

        // Update guest session balance
        await tx
          .update(guestSession)
          .set({
            balance: newBalance.toFixed(2),
          })
          .where(eq(guestSession.sessionId, sessionId));

        console.log("Guest credits deducted successfully (database)", {
          sessionId,
          amount: roundedAmount,
          newBalance,
        });

        return newBalance;
      });

      return result;
    } catch (error) {
      if (error instanceof ChatSDKError) {
        throw error;
      }
      console.error("Failed to deduct guest credits", {
        sessionId,
        amount: roundedAmount,
        error: error instanceof Error ? error.message : String(error),
      });
      throw new ChatSDKError(
        "bad_request:database",
        "Failed to deduct guest credits"
      );
    }
  }, `deductGuestCredits for session ${sessionId}, amount ${roundedAmount}`);
}

/**
 * Add credits to an authenticated user's balance.
 * Uses database transaction with row-level locking to prevent race conditions.
 * Supports types: purchase, bonus, monthly_allowance
 *
 * @param params - Addition parameters
 * @param params.userId - The user's ID
 * @param params.amount - The amount of credits to add
 * @param params.type - The type of credit addition (purchase, bonus, monthly_allowance)
 * @param params.description - Description of the transaction
 * @param params.metadata - Optional metadata for the transaction
 * @returns The created credit transaction record
 * @throws ChatSDKError if invalid parameters or database error
 */
export async function addCredits(params: {
  userId: string;
  amount: number;
  type: "purchase" | "bonus" | "monthly_allowance";
  description: string;
  metadata?: Record<string, unknown>;
}): Promise<CreditTransaction> {
  const { userId, amount, type, description, metadata } = params;

  // Validate amount is positive
  if (amount <= 0) {
    console.warn("Attempted to add non-positive credits", { userId, amount });
    throw new ChatSDKError(
      "bad_request:database",
      "Credit addition amount must be positive"
    );
  }

  // Validate type
  const validTypes = ["purchase", "bonus", "monthly_allowance"];
  if (!validTypes.includes(type)) {
    console.warn("Invalid credit type", { userId, type });
    throw new ChatSDKError(
      "bad_request:database",
      `Invalid credit type. Must be one of: ${validTypes.join(", ")}`
    );
  }

  // Round amount to 2 decimal places
  const roundedAmount = Math.round(amount * 100) / 100;

  return retryDatabaseOperation(async () => {
    try {
      // Use database transaction with row-level locking
      const result = await db.transaction(async (tx) => {
        // Lock the user's credit balance row for update
        const [balance] = await tx
          .select()
          .from(creditBalance)
          .where(eq(creditBalance.userId, userId))
          .for("update");

        if (!balance) {
          console.warn("Credit balance not found during addition", { userId });
          throw new ChatSDKError(
            "not_found:database",
            "Credit balance not found for user"
          );
        }

        // Convert balance to number for calculation
        const currentBalance = Number.parseFloat(balance.balance);

        // Calculate new balance
        const newBalance =
          Math.round((currentBalance + roundedAmount) * 100) / 100;

        // Update credit balance
        await tx
          .update(creditBalance)
          .set({
            balance: newBalance.toFixed(2),
            updatedAt: new Date(),
          })
          .where(eq(creditBalance.userId, userId));

        // Record transaction
        const [transaction] = await tx
          .insert(creditTransaction)
          .values({
            userId,
            type,
            amount: roundedAmount.toFixed(2),
            balanceAfter: newBalance.toFixed(2),
            description,
            metadata: metadata ? metadata : null,
          })
          .returning();

        console.log("Credits added successfully", {
          userId,
          type,
          amount: roundedAmount,
          newBalance,
          transactionId: transaction.id,
        });

        return transaction;
      });

      // Invalidate cache after successful addition
      await invalidateBalanceCache(userId);

      return result;
    } catch (error) {
      if (error instanceof ChatSDKError) {
        throw error;
      }
      console.error("Failed to add credits", {
        userId,
        type,
        amount: roundedAmount,
        error: error instanceof Error ? error.message : String(error),
      });
      throw new ChatSDKError("bad_request:database", "Failed to add credits");
    }
  }, `addCredits for user ${userId}, type ${type}, amount ${roundedAmount}`);
}

/**
 * Allocate new user bonus credits (1000 credits).
 * Checks isNewUser flag to prevent duplicate allocation.
 *
 * @param userId - The user's ID
 * @throws ChatSDKError if user not found, already received bonus, or database error
 */
export async function allocateNewUserBonus(userId: string): Promise<void> {
  return retryDatabaseOperation(async () => {
    try {
      // Use database transaction to ensure atomicity
      await db.transaction(async (tx) => {
        // Lock the user's credit balance row for update
        const [balance] = await tx
          .select()
          .from(creditBalance)
          .where(eq(creditBalance.userId, userId))
          .for("update");

        if (!balance) {
          console.warn("Credit balance not found for new user bonus", {
            userId,
          });
          throw new ChatSDKError(
            "not_found:database",
            "Credit balance not found for user"
          );
        }

        // Check if user has already received the bonus
        if (!balance.isNewUser) {
          console.warn("User already received new user bonus", { userId });
          throw new ChatSDKError(
            "bad_request:database",
            "User has already received the new user bonus"
          );
        }

        // Convert balance to number for calculation
        const currentBalance = Number.parseFloat(balance.balance);

        // Add 1000 credits
        const bonusAmount = 1000.0;
        const newBalance =
          Math.round((currentBalance + bonusAmount) * 100) / 100;

        // Update credit balance and set isNewUser to false
        await tx
          .update(creditBalance)
          .set({
            balance: newBalance.toFixed(2),
            isNewUser: false,
            updatedAt: new Date(),
          })
          .where(eq(creditBalance.userId, userId));

        // Record transaction
        await tx.insert(creditTransaction).values({
          userId,
          type: "bonus",
          amount: bonusAmount.toFixed(2),
          balanceAfter: newBalance.toFixed(2),
          description: "New user bonus",
          metadata: null,
        });

        console.log("New user bonus allocated successfully", {
          userId,
          bonusAmount,
          newBalance,
        });
      });

      // Invalidate cache after successful bonus allocation
      await invalidateBalanceCache(userId);
    } catch (error) {
      if (error instanceof ChatSDKError) {
        throw error;
      }
      console.error("Failed to allocate new user bonus", {
        userId,
        error: error instanceof Error ? error.message : String(error),
      });
      throw new ChatSDKError(
        "bad_request:database",
        "Failed to allocate new user bonus"
      );
    }
  }, `allocateNewUserBonus for user ${userId}`);
}

/**
 * Allocate monthly credits (200 credits).
 * Checks lastMonthlyAllocation to prevent duplicate allocation in the same month.
 *
 * @param userId - The user's ID
 * @throws ChatSDKError if user not found, already received allocation this month, or database error
 */
export async function allocateMonthlyCredits(userId: string): Promise<void> {
  return retryDatabaseOperation(async () => {
    try {
      // Use database transaction to ensure atomicity
      await db.transaction(async (tx) => {
        // Lock the user's credit balance row for update
        const [balance] = await tx
          .select()
          .from(creditBalance)
          .where(eq(creditBalance.userId, userId))
          .for("update");

        if (!balance) {
          console.warn("Credit balance not found for monthly allocation", {
            userId,
          });
          throw new ChatSDKError(
            "not_found:database",
            "Credit balance not found for user"
          );
        }

        // Check if user has already received monthly allocation this month
        const now = new Date();
        if (balance.lastMonthlyAllocation) {
          const lastAllocation = new Date(balance.lastMonthlyAllocation);

          // Check if we're in the same month and year
          if (
            lastAllocation.getMonth() === now.getMonth() &&
            lastAllocation.getFullYear() === now.getFullYear()
          ) {
            console.warn(
              "User already received monthly allocation this month",
              {
                userId,
                lastAllocation: lastAllocation.toISOString(),
              }
            );
            throw new ChatSDKError(
              "bad_request:database",
              "User has already received monthly allocation for this month"
            );
          }
        }

        // Convert balance to number for calculation
        const currentBalance = Number.parseFloat(balance.balance);

        // Add 200 credits
        const monthlyAmount = 200.0;
        const newBalance =
          Math.round((currentBalance + monthlyAmount) * 100) / 100;

        // Update credit balance and lastMonthlyAllocation timestamp
        await tx
          .update(creditBalance)
          .set({
            balance: newBalance.toFixed(2),
            lastMonthlyAllocation: now,
            updatedAt: new Date(),
          })
          .where(eq(creditBalance.userId, userId));

        // Record transaction
        await tx.insert(creditTransaction).values({
          userId,
          type: "monthly_allowance",
          amount: monthlyAmount.toFixed(2),
          balanceAfter: newBalance.toFixed(2),
          description: "Monthly credit allowance",
          metadata: null,
        });

        console.log("Monthly credits allocated successfully", {
          userId,
          monthlyAmount,
          newBalance,
          allocationDate: now.toISOString(),
        });
      });

      // Invalidate cache after successful monthly allocation
      await invalidateBalanceCache(userId);
    } catch (error) {
      if (error instanceof ChatSDKError) {
        throw error;
      }
      console.error("Failed to allocate monthly credits", {
        userId,
        error: error instanceof Error ? error.message : String(error),
      });
      throw new ChatSDKError(
        "bad_request:database",
        "Failed to allocate monthly credits"
      );
    }
  }, `allocateMonthlyCredits for user ${userId}`);
}

/**
 * Get transaction history for an authenticated user.
 * Returns transactions in reverse chronological order (newest first).
 * Supports pagination with limit and offset.
 *
 * @param params - Query parameters
 * @param params.userId - The user's ID
 * @param params.limit - Maximum number of transactions to return (default: 10)
 * @param params.offset - Number of transactions to skip (default: 0)
 * @returns Array of credit transactions
 * @throws ChatSDKError if database error occurs
 */
export async function getTransactionHistory(params: {
  userId: string;
  limit?: number;
  offset?: number;
}): Promise<CreditTransaction[]> {
  const { userId, limit = 10, offset = 0 } = params;

  return retryDatabaseOperation(async () => {
    try {
      const transactions = await db
        .select()
        .from(creditTransaction)
        .where(eq(creditTransaction.userId, userId))
        .orderBy(desc(creditTransaction.createdAt))
        .limit(limit)
        .offset(offset);

      return transactions;
    } catch (error) {
      console.error("Failed to get transaction history", {
        userId,
        limit,
        offset,
        error: error instanceof Error ? error.message : String(error),
      });
      throw new ChatSDKError(
        "bad_request:database",
        "Failed to retrieve transaction history"
      );
    }
  }, `getTransactionHistory for user ${userId}`);
}
